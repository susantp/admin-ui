...
export const getGroupedData = ({
  permissions,
}: IGetGroupedData): IGroupedScreenWithPermissions[] =>
  permissions.reduce<IGroupedScreenWithPermissions[]>(
    (acc: IGroupedScreenWithPermissions[], obj: IPermission) => {
      const { screen, id, code } = obj
      const existingEntry: IGroupedScreenWithPermissions | undefined = acc.find(
...
export const getRoleFormSchema = (): Yup.ObjectSchema<IRoleFormValues> =>
  Yup.object().shape({
    name: Yup.string()
      .min(2, "Too Short!")
      .max(50, "Too Long!")
...
export const getHelperTexts = ({ slug }: IGetHelperTextsParams): IHelperTexts =>
  slug
    ? {
        formTitle: "Edit Role",
        formSubtitle:
          "Get started by updating the information below to update the role.",
    ...
    : {
        ...
        formSubtitle:
          "Get started by filling in the information below to create a new role.",
...
export const getPermissionIds = ({
  permissions,
}: IGetPermissionIdsProps): string[] =>
  permissions.reduce((ids: string[], item) => {
    ids.push(item.id)
    return ids
  }, [])
