...
import Restricted from "@/src/modules/global/presentation/components/restricted";
...
export default function RoleContainer(): React.ReactNode {
    ...
    if (loading || !currentScreen || !roles || !roles.results.length) return <PocLoader/>
    ...
    return (
        <PocContainer>
            <Section
                ...
                actionEl={<Restricted to="CREATE"><ActionLink path={appPaths.createRole.path("")}/></Restricted>}
            >
                <PocRoleListWrapper>
                    {results.map((role) => (
