...
function ScreenPermissionBox(): JSX.Element {
  return (
    <div className="flex flex-col gap-y-3">
      <div id="screen-title" className="flex justify-center p-2 bg-secondary">
        <p>Screen</p>
      </div>
      ...
      {[{name: "Create", status: true}, {
        ...
        <div id="permissionDiv" className="flex gap-x-5 px-4">
          <input type="checkbox" id="p-1" checked={item.status}
                 className="flex-none"/>
          <p id="permission-name" className="flex-1">{item.name}</p>
        </div>
      ))}
      ...
      <Section label="" subLabel={null} actionEl={null}>
        <form
          ...
          <div className="h-0 flex-1 overflow-y-auto">
            ...
            <div id="screen-permission-wrapper"
                 className="flex flex-col w-full sm:px-6 gap-y-4">
              <div id="wrapper-title"
                   className="bg-primary text-white py-6 items-center justify-center flex">
                <h2 className="text-lg">Manage Permissions</h2>
              </div>

              <div
                id="screens"
                className="grid grid-cols-3 items-center justify-center gap-6">
                {[1, 2, 3, 4].map(item => <ScreenPermissionBox/>)}
              </div>
